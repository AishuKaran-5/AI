...
let qrSent = false; // QR code send hone ka flag

client.on('qr', async qr => {
    if (qrSent) return;  // Agar pehle hi send ho chuka hai toh dobara nahi bhejega

    console.log("QR Code URL:", qr);  
    qrSent = true; // QR send ho chuka hai, dobara nahi bhejega
...
// Jab client ready ho jaye (login complete ho jaye) toh flag reset kar do
client.on('ready', () => {
    console.log('Bot is ready to use.. Listening for messages!');
    qrSent = false; // Login hone ke baad dobara QR send karne ke liye reset
});
